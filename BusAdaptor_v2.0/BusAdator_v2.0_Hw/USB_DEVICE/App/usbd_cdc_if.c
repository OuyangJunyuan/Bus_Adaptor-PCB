...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 11 */
	...
	USBD_CDC_SetRxBuffer(&hUsbDeviceHS, frameptr->data);
...
void usb_printf(const char *format, ...)
{
    ...
    uint8_t UserTxBufferFS[CDC_DATA_HS_MAX_PACKET_SIZE];
    length = vsnprintf((char *)UserTxBufferFS, CDC_DATA_HS_MAX_PACKET_SIZE, (char *)format, args);
    va_end(args);
    CDC_Transmit_HS(UserTxBufferFS, length);
}
